...
def main():
    ...
    # STEP 6: Prompt user for history --> display if entry == 'y'
    ...
    if sShowHistory == 'y':
        ...
        for sStoredName, weights in dictPlanetHistory.items():
            print(f"\n{sStoredName}'s weights:")
            for sPlanet, fWeight in weights.items():
                print(f'Weight on {sPlanet:10} {fWeight}')
        print('-------------------------------------')

    # STEP 7: Prompt for user name & validate uniqueness (sName)
    while True:
        ...
        ##> 9.2: Save it to dictPlanetHistory (sName=key, dictPersonWeights=value)
